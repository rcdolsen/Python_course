...
# Directions = enum.Enum('Direcoes', ['ESQUERDA', 'DIREITA']) # O python fica
# sem saber o tipo de Directions

class Directions(enum.Enum):
    ESQUERDA = enum.auto() # pode ser definido manualmente. ex: 1
    DIREITA = enum.auto()

print(Directions(1), Directions['ESQUERDA'], Directions.ESQUERDA)
print(Directions(1).name, Directions.ESQUERDA.value)

def mover(direcao: Directions):
    if not isinstance(direcao, Directions):
